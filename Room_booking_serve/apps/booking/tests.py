...
class RoomManagementTests(TestCase):
    ...
    def test_add_room(self):
        response = self.client.post(
            "/api/v1/meeting-rooms/add-room/",
            {"room_name": "Test Room", "capacity": 8},
            format="json",
        )

        self.assertEqual(response.status_code, 201)
        self.assertEqual(MeetingRoom.objects.count(), 1)
        self.assertEqual(MeetingRoom.objects.first().room_name, "Test Room")

    def test_update_room_capacity(self):
        room = MeetingRoom.objects.create(room_name="Room X", capacity=5)

        response = self.client.patch(
            f"/api/v1/meeting-rooms/{room.id}/update-capacity/",
            {"capacity": 12},
            format="json",
        )
